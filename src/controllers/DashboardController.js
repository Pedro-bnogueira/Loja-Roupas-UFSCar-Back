...
const getDashboardStats = async (req, res) => {
    ...
    try {
        ...
        // Total de Vendas
        const totalSales = (await TransactionHistory.sum("transactionPrice", {
            where: { type: "out" },
            transaction: t
        })) || 0;

        // Total de Compras
        const totalPurchases = (await TransactionHistory.sum("transactionPrice", {
            where: { type: "in" },
            transaction: t
        })) || 0;
        ...
        const topProductQuery = await TransactionHistory.findOne({
            ...
            include: [{
                model: Product,
                as: "product",
                attributes: ['id', 'name', 'brand', 'price', 'size', 'color', 'categoryId']
            }],
            transaction: t
        });
        ...
        const timeSeriesConfig = (type) => ({
            attributes: [
                ...
                [sequelize.fn("SUM", sequelize.col("transactionPrice")), "total"]
            ],
            where: { 
                type,
        ...
        const inventoryByCategory = await Stock.findAll({
            attributes: [
                [sequelize.literal('`product->category`.`name`'), 'categoryName'],
                [sequelize.fn('SUM', 
                    sequelize.literal('CASE WHEN Stock.operationType = "in" THEN quantity ELSE -quantity END')
                ), 'totalStock']
            ],
            include: [{
                model: Product,
        ...
        const usersOverTime = await User.findAll({
            attributes: [
                [sequelize.fn("DATE_FORMAT", sequelize.col("createdDate"), "%Y-%m"), "month"],
        ...
        const salesOverTime = processTimeData(rawSales);
        const purchasesOverTime = processTimeData(rawPurchases);
